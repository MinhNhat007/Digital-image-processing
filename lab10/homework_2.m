...
%% processing
I_dom = imread('wDemo.png');

numFigure = numFigure + 1;
figure(numFigure);
nx = 1; ny = 2;
[counts,binLocations] = imhist(I_dom);
subplot(nx, ny, 1); imshow(I_dom); title('original');
subplot(nx, ny, 2); imshow(imhist(I_dom)); title('histogram');

IC = double(I_dom);
for n = 2 : 257
    if (n == 257) || (counts(n) > n)
        Q = find(IC < n);
        [I_label, num] = bwlabel(IC);
        Cn = Q;
        Cnn = Cn;
    end
end
